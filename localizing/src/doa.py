...
def build_mic_positions(
    radius_m: float, angles_deg: Tuple[float, float, float, float], origin_xyz: Tuple[float, float, float] = (0.0, 0.0, 0.0)
) -> np.ndarray:
    """
    Returns mic positions (4, 3) in meters on XY plane (z=0) offset by origin_xyz.
    """
    positions = []
    origin = np.array(origin_xyz, dtype=np.float64)
