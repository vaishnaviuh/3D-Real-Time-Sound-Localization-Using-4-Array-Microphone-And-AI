...
smoothed_direction_state: Optional[np.ndarray] = None
smoothed_confidence_state: float = 0.0
smoothed_position_state: Optional[np.ndarray] = None
...
def _smooth_direction_vector(new_vec: np.ndarray, confidence: float) -> tuple[np.ndarray, float]:
    global smoothed_direction_state, smoothed_confidence_state
    new_unit = _unit_vector(new_vec)

    alpha = np.clip(config.detection.smoothing_alpha, 0.0, 1.0)
    if smoothed_direction_state is None or alpha <= 0.0:
        smoothed_direction_state = new_unit
        smoothed_confidence_state = max(confidence, config.detection.min_fused_confidence)
        return smoothed_direction_state, smoothed_confidence_state

    prev_unit = smoothed_direction_state
    ...
    if (
        ...
        return prev_unit, smoothed_confidence_state

    blended = _unit_vector(alpha * new_unit + (1 - alpha) * prev_unit)
    smoothed_direction_state = blended
    smoothed_confidence_state = max(confidence, smoothed_confidence_state * 0.8, config.detection.min_fused_confidence)
    return smoothed_direction_state, smoothed_confidence_state


def _smooth_position_vector(new_position: Optional[np.ndarray], confidence: float) -> Optional[np.ndarray]:
    global smoothed_position_state
    if new_position is None:
        return smoothed_position_state

    smoothing = np.clip(config.detection.smoothing_alpha, 0.0, 1.0)
    new_pos = np.array(new_position, dtype=np.float64)
    if smoothed_position_state is None or smoothing <= 0.0:
        smoothed_position_state = new_pos
        return smoothed_position_state

    smoothed_position_state = smoothing * new_pos + (1 - smoothing) * smoothed_position_state
    return smoothed_position_state
