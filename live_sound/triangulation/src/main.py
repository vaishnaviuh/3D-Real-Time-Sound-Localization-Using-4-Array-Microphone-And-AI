...
from src.audio_io import save_audio_to_wav
...
def _record_array_audio(cfg: AppConfig, array_cfg: ArraySetup, mic_positions: np.ndarray) -> np.ndarray:
    return record_multichannel(
        ...
        use_simulation=cfg.simulation.enable_simulation,
...
def run_once(cfg: AppConfig) -> None:
    ...
    for entry in array_geometries:
        ...
        try:
            signals = _record_array_audio(cfg, array_cfg, mic_positions)
    ...
    if fused_position is None:
        fused_position = reference_result.get("position_vector")
    ...
    print(
        f"FUSED -> Azimuth: {fused_az:.1f}deg, "
        f"Elevation: {fused_el:.1f}deg, "
    ...
    for res in successful_results:
        ...
        print(
            f"  [{res['array_name']}] Az={res['azimuth_deg']:.1f}deg, "
            f"El={res['elevation_deg']:.1f}deg, Dist={dist_txt}, Conf={res['confidence']:.2f}"
        )
    ...
    if cfg.saving.enable_save_audio or cfg.saving.enable_save_results:
        ...
        if cfg.saving.enable_save_audio:
            for res in successful_results:
                # Save as NumPy array (original format)
                filename_npy = f"audio_{res['array_name'].replace(' ', '_')}_{ts}.npy"
                np.save(os.path.join(cfg.saving.output_dir, filename_npy), res["signals"])
                
                # Also save as WAV file for easier playback/analysis
                filename_wav = f"audio_{res['array_name'].replace(' ', '_')}_{ts}.wav"
                wav_path = os.path.join(cfg.saving.output_dir, filename_wav)
                # signals shape: (num_samples, num_channels)
                save_audio_to_wav(
                    audio_data=res["signals"],
